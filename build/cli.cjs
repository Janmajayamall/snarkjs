...
async function readHeaderPlonk(fd, sections, toObject) {
	...
	zkey.X_1 = await readG1(fd, zkey.curve, toObject);
...
async function readZKey(fileName, toObject) {
	const { fd, sections } = await binFileUtils__namespace.readBinFile(
		fileName,
		"zkey",
		1
	);
...
async function writeContribution(fd, curve, c) {
	...
	if (c.name) {
		params.push(1); // Param Name
		const nameData = new TextEncoder("utf-8").encode(
			c.name.substring(0, 64)
		);
		params.push(nameData.byteLength);
		for (let i = 0; i < nameData.byteLength; i++) params.push(nameData[i]);
	}
...
async function plonkFullProve$1(
	_input,
	wasmFile,
	zkeyFileName,
	logger
...
async function groth16Prove(params, options) {
	...
	const { proof, publicSignals } = await groth16Prove$1(
		zkeyName,
		witnessName,
		logger
	);
...
async function groth16FullProve(params, options) {
	...
	const { proof, publicSignals } = await groth16FullProve$1(
		input,
		wasmName,
		zkeyName,
		logger
...
async function groth16Verify(params, options) {
	...
	const verificationKey = JSON.parse(
		fs__default["default"].readFileSync(verificationKeyName, "utf8")
	);
...
async function fileExists(file) {
	return fs__default["default"].promises
		.access(file, fs__default["default"].constants.F_OK)
		.then(() => true)
		.catch(() => false);
}
...
async function zkeyExportSolidityVerifier(params, options) {
	...
	if (await fileExists(path__default["default"].join(__dirname$1, "templates"))) {
		templates.groth16 = await fs__default["default"].promises.readFile(
			path__default["default"].join(__dirname$1, "templates", "verifier_groth16.sol.ejs"),
			"utf8"
		);
		templates.plonk = await fs__default["default"].promises.readFile(
			path__default["default"].join(__dirname$1, "templates", "verifier_plonk.sol.ejs"),
			"utf8"
		);
	} else {
		templates.groth16 = await fs__default["default"].promises.readFile(
	...
	const verifierCode = await exportSolidityVerifier(
		zkeyName,
		templates);
...
async function powersOfTauChallengeContribute(params, options) {
	...
	return await challengeContribute(
		curve,
		challengeName,
		responseName,
		options.entropy,
...
async function zkeyImportBellman(params, options) {
	...
	const isValid = await phase2importMPCParams(
		zkeyNameOld,
		mpcParamsName,
		zkeyNameNew,
		options.name,
...
async function zkeyVerifyFromInit(params, options) {
	...
	const res = await phase2verifyFromInit(
		initZKeyName,
		ptauName,
		zkeyName,
		logger
...
async function plonkProve(params, options) {
	...
	const { proof, publicSignals } = await plonk16Prove(
		zkeyName,
		witnessName,
		logger
	);
...
async function fileInfo(params) {
	...
	try {
		const { fd: fd, sections: sections } = await binFileUtils__namespace.readBinFile(
			filename,
			extension,
			2,
			1 << 25,
		...
		sections.forEach((section, index) => {
			...
			if (section.length > 1)
				errors.push(
					`Section ${index} has more than one section definition`
				);
			else {
				if (section[0].size === 0) {
					errors.push(
			...
			const color =
				errors.length === 0 ? "%s%s%s" : "%s\x1b[31m%s\x1b[0m%s";
