...
async function powersOfTauChallengeContribute(params, options) {
	...
	return await powersOfTau.challengeContribute(
		curve,
		challengeName,
		responseName,
		options.entropy,
...
async function powersOfTauImport(params, options) {
	...
	const res = await powersOfTau.importResponse(
		oldPtauName,
		response,
		newPtauName,
		options.name,
...
async function powersOfTauBeacon(params, options) {
	...
	return await powersOfTau.beacon(
		oldPtauName,
		newPtauName,
		options.name,
		beaconHashStr,
...
async function powersOfTauContribute(params, options) {
	...
	return await powersOfTau.contribute(
		oldPtauName,
		newPtauName,
		options.name,
		options.entropy,
...
async function plonkSetupMaze(params, options) {
	...
	const inputs = JSON.parse(await fs.promises.readFile(inputsName, "utf8"));
	...
	await bfj.write(proofsName, stringifyBigInts(proofs), { space: 1 });
	await bfj.write(publicsName, stringifyBigInts(publicSignalsArr), {
		space: 1,
	});

	const vKey = await zkey.exportVerificationKey(zkeyName);
	await bfj.write(verificationName, stringifyBigInts(vKey), {
		space: 1,
	});
